...
func (e *MixingPitches) Apply(sys *internal.System, t time.Time) {
...
func (e *MixingPitches) distance(x, y float64, index int) float64 {
	...
	// Our circle should actually have x-coords and y-coords in the range
	// [0, 1], so we have to recentre our circle at 0.5 with a radius of 0.5
	circle_x := (math.Cos(angle) + 1) / 2
	circle_y := (math.Sin(angle) + 1) / 2

	x_diff := x - circle_x
	y_diff := y - circle_y

	result := math.Sqrt((x_diff*x_diff + y_diff*y_diff) / 2)

	if result < 0 || result > 1 {
		fmt.Println("Out of bounds:", result, x_diff, y_diff)
	}
